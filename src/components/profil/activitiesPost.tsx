...
export default function UserActivityList() {
  ...
  return (
    <div className="space-y-4">
      {activities.map((act) => (
        <div key={act.id} className="border rounded-lg p-4 shadow-sm bg-white">
          <Image
            src={act.image_url ?? "/default-image.png"}
            alt={act.title}
            width={400}
            height={250}
          ...
          <div className="mt-2 text-sm">
            <p>
              <strong>Kategori:</strong> {act.activity_categories.name}
            </p>
            <p>
              <strong>Grup:</strong> {act.activity_categories.group_category}
            </p>
            <p>
              <strong>Poin:</strong> {act.points}
            </p>
          </div>
        </div>
      ))}
