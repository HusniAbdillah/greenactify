...
type ProfileActivityProps = {
  activities: ActivityItem[]; // ganti `Activity` sesuai nama tipe data kamu
  loading: boolean;
  error: Error | null;
  refetch: () => void;
};
export default function ProfileActivity({
  ...
  const activitiesData = [
    { name: 'daur_ulang', count: countMap['Daur Ulang'], icon: <Recycle className="h-10 w-10 text-emerald-500" />, label: 'Daur Ulang', description: 'Mengubah sampah menjadi sesuatu yang baru dan bermanfaat, mengurangi limbah.' },
    { name: 'bersih_bersih', count: countMap['Bersih-bersih'], icon: <BrushCleaning className="h-10 w-10 text-blue-500" />, label: 'Bersih-Bersih', description: 'Menjaga lingkungan tetap rapi dan bebas dari sampah, untuk kesehatan bersama.' },
    { name: 'hemat_energi', count: countMap['Hemat Energi'], icon: <Lightbulb className="h-10 w-10 text-amber-500" />, label: 'Hemat Energi', description: 'Mengurangi konsumsi energi untuk melestarikan sumber daya dan biaya.' },
    { name: 'edukasi_lingkungan', count: countMap['Edukasi Lingkungan'], icon: <Book className="h-10 w-10 text-purple-500" />, label: 'Edukasi Lingkungan', description: 'Meningkatkan kesadaran dan pengetahuan tentang isu-isu lingkungan.' },
    { name: 'hemat_air', count: countMap['Hemat Air'], icon: <Droplet className="h-10 w-10 text-cyan-500" />, label: 'Hemat Air', description: 'Mengelola penggunaan air secara bijak untuk keberlanjutan pasokan air bersih.' },
    { name: 'makanan_organik', count: countMap['Makanan Organik'], icon: <Wheat className="h-10 w-10 text-lime-500" />, label: 'Makanan Organik', description: 'Mendukung pertanian berkelanjutan dan mengurangi paparan bahan kimia berbahaya.' },
    { name: 'transportasi_hijau', count: countMap['Transportasi Hijau'], icon: <Bike className="h-10 w-10 text-indigo-500" />, label: 'Transportasi Hijau', description: 'Memilih moda transportasi ramah lingkungan untuk mengurangi emisi karbon.' },
    { name: 'penghijauan', count: countMap['Penghijauan'], icon: <Trees className="h-10 w-10 text-green-500" />, label: 'Penghijauan', description: 'Menanam pohon dan menjaga vegetasi untuk udara bersih dan ekosistem sehat.' },
  ];
  ...
  return (
    <>
      <section className="mx-auto flex bg-oliveSoft rounded-4xl p-1 mt-5 max-w-7xl">
        <button
          className={`flex-1 py-3 px-4 rounded-4xl font-semibold transition-colors ${activeTab === 'overview' ? 'bg-greenDark text-whiteMint shadow-sm' : 'text-greenDark hover:bg-mintPastel hover:shadow-lg'}`}
          onClick={() => setActiveTab('overview')}
        >
          Jejakku
        </button>
        <button
          className={`flex-1 py-3 px-4 rounded-4xl font-semibold transition-colors ${activeTab === 'activities' ? 'bg-greenDark text-whiteMint shadow-sm' : 'text-greenDark hover:bg-mintPastel hover:shadow-lg'}`}
